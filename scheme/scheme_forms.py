...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # lambda_expression = Pair(formals, expressions.rest)
        # function = do_lambda_form(lambda_expression, env)
        # env.define(function_name, function)
        # return function_name
        name = signature.first
        lambda_exp = Pair("lambda", Pair(signature.rest, expressions.rest))
...
def do_and_form(expressions, env):
    ...
    res = True
    while cur != nil:
        res = scheme_eval(cur.first, env, True)
...
def make_let_frame(bindings, env):
    ...
    pairs = bindings
    while pairs != nil:
        validate_form(pairs.first, 2, 2)
        name, val = pairs.first.first, scheme_eval(pairs.first.rest.first, env)
