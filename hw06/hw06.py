...
class VendingMachine:
    ...
    def __init__(self, product, price):
        assert isinstance(product, str) and (isinstance(price, int) or isinstance(price, float))
...
def two_list(vals, counts):
    ...
    """
    "*** YOUR CODE HERE ***"
    assert isinstance(vals, list) and isinstance(counts, list)
    if len(vals) == len(counts) == 0:
        return Link.empty
    else:
        counts[0] -= 1
        if counts[0] == 0:
