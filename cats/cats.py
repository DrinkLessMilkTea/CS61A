...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def check(para):
        for word in split(remove_punctuation(para)):
            word = word.lower()
            if word in subject:
                return True
        return False
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    res = [w for w in word_list if diff_function(typed_word,w,limit) <= limit]
    if len(res) == 0:
        return typed_word
    return min(res,key=diff)
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    def change(t,s,l,cnt):
        ...
        if len(s) == 0:
            return cnt + len(t)
        if t[0] != s[0]:
            return change(t[1:],s[1:],l,cnt + 1)
        else:
            return change(t[1:],s[1:],l,cnt)
    return change(typed,source,limit,0)
    # END PROBLEM 6
...
def minimum_mewtations(typed, source, limit):
    ...
    def change(t,s,l,cnt):
        ...
        if len(s) == 0:
            return cnt + len(t)
        if t[0] != s[0]:
            add = change(t,s[1:],l,cnt + 1)
            remove = change(t[1:],s,l,cnt + 1)
            sub = change(t[1:],s[1:],l,cnt + 1)
...
def fastest_words(match):
    ...
    res = [[] for i in range(len(times))]
    
    for word_index in range(len(get_all_words(match))):
        fastest = 1e5
        who = -1
        for player in range(len(times)):
            if time(match,player,word_index) < fastest:
                fastest = time(match,player,word_index)
                who = player
        res[who] += [get_word(match,word_index)]
    return res
    # END PROBLEM 10
