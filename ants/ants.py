...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            origin_ant: ContainerAnt | Ant = place.ant
            if origin_ant.is_container and origin_ant.can_contain(self):
                origin_ant.store_ant(self)
            elif self.is_container and self.can_contain(origin_ant):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while True:
            if cur.is_hive:
                return None
            target = random_bee(cur.bees)
            if target and dist >= self.lower_bound and dist <= self.upper_bound:
                return target
            cur = cur.entrance
            dist += 1
        # END Problem 3 and 4
...
class LaserAnt(ThrowerAnt):
        ...
        while True:
            ...
            if cur.ant:
                ...
                if cur.ant.is_container and cur.ant.ant_contained:
                    res[cur.ant.ant_contained] = dist
            for bee in cur.bees:
                res[bee] = dist
            dist, cur = dist + 1, cur.entrance
        res.pop(self)
        return res
        # END Problem Optional 2
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        hurt = self.damage - 0.25 * distance - 0.0625 * self.insects_shot
        if hurt < 0:
            return 0
        return hurt
        # END Problem Optional 2
